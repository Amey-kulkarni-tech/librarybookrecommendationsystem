...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($stud_id) {
        ...
        if (!empty($book_ids)) {
            // Create temporary table for liked books
            $createTempTableQuery = "CREATE TEMPORARY TABLE IF NOT EXISTS liked_books_temp AS (
                SELECT DISTINCT bm.id AS liked_book_id
                FROM book_master1 bm
                JOIN stud_book_map sbm ON bm.id = sbm.book_id
                JOIN login_master lm ON sbm.stud_id = lm.stud_id
                WHERE lm.token = ?
                AND sbm.islike = 1
            )";

            $stmtCreateTempTable = $con->prepare($createTempTableQuery);
            if (!$stmtCreateTempTable) {
                die("Error preparing create temp table query: " . $con->error);
            }

            $stmtCreateTempTable->bind_param("s", $token);
            $stmtCreateTempTable->execute();
            $stmtCreateTempTable->close(); // Close the statement after creating the temp table

           
            $relatedBooks = array();

            // Execute the provided SQL query for book recommendations
            $recommendationQuery = "
                SELECT DISTINCT bm.id, bm.bname, bm.aname, bm.price
                FROM book_master1 bm
                JOIN tag_map tm ON bm.id = tm.bid
                JOIN tag_master t ON tm.tid = t.id
                JOIN stud_book_map sbm ON bm.id = sbm.book_id
                JOIN login_master lm ON sbm.stud_id = lm.stud_id
                WHERE lm.token = ?
                AND (sbm.islike = 1 OR (sbm.islike = 0 AND bm.id IN (SELECT liked_book_id FROM liked_books_temp)))
                ORDER BY RAND()
                LIMIT 10;
            ";
            ...
            $stmtRecommendation = $con->prepare($recommendationQuery);
            if (!$stmtRecommendation) {
                die("Error preparing recommendation query: " . $con->error);
            }
            
            $stmtRecommendation->bind_param("s", $token);
            $stmtRecommendation->execute();
            $resultRecommendation = $stmtRecommendation->get_result();
            
            while ($rowRecommendation = $resultRecommendation->fetch_assoc()) {
                // Fetch associated tags for each suggested book
                ...
                $stmtTags->bind_param("i", $rowRecommendation['id']);
                $stmtTags->execute();
                ...
                // Get islike status for the current suggested book
                $islike = getIsLikeStatus($book_likes, $rowRecommendation['id']);
            
                // Store suggested book details along with tags and islike status in the suggestions array
                $relatedBooks[] = array(
                    "id" => $rowRecommendation['id'],
                    "bname" => $rowRecommendation['bname'],
                    "aname" => $rowRecommendation['aname'],
                    "price" => $rowRecommendation['price'],
                    "tags" => $tags,
                    "islike" => $islike
                );
            }
            
            $stmtRecommendation->close(); // Close the statement after fetching recommendations
            ...
            // while ($rowRecommendation = $resultRecommendation->fetch_assoc()) {
            //     $relatedBooks[] = $rowRecommendation;
            // }
...
//             // Execute the provided SQL query for book recommendations
//             $recommendationQuery = "
//                 -- Replace 'your_user_token' with the actual token value for which you want to make recommendations.
//                 SET @user_token = ?;

//                 -- Select books with similar tags to those liked by the user, including the liked books.
//                 SELECT DISTINCT bm.id, bm.bname, bm.aname, bm.price
//                 FROM book_master1 bm
//                 JOIN tag_map tm ON bm.id = tm.bid
//                 JOIN tag_master t ON tm.tid = t.id
//                 JOIN stud_book_map sbm ON bm.id = sbm.book_id
//                 JOIN login_master lm ON sbm.stud_id = lm.stud_id
//                 WHERE lm.token = @user_token
//                 AND (sbm.islike = 1 OR (sbm.islike = 0 AND bm.id IN (
//                     -- Subquery to get books liked by the user
